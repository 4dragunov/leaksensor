...
#define ISR_STACK_LENGTH_BYTES ((uint32_t)&_Min_Stack_Size) // bytes to reserve for ISR (MSP) stack
...
#define HEAP_REGIONS  {{ (uint8_t *) (uint32_t)&__HeapBase,  (uint8_t *) (uint32_t)&__HeapLimit },\
                       { (uint8_t *) 0x10000000, 0x10007FFF}};
...
static void prvHeapInit(uint32_t i)
{
  ...
  size_t xTotalHeapSize = ucHeap[i].pucEndAddress - ucHeap[i].pucStartAddress;
...
int32_t multiRegionSearch(void * pv)
{
  ...
  for(uint32_t i = 0; i < HEAP_NUM; i++)
  {
    ...
    if(pv >= p && pv < (p + (ucHeap[i].pucEndAddress - ucHeap[i].pucStartAddress)))
    {
      return i;
    }
  }
