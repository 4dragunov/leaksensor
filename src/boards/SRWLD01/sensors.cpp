...
DataSampler::Channels DataSampler::mChannels ={
		...
		Channel(20, gChannelsPins[20],  DataSampler::mSamples.data.raw[20],  Channel::Type::TS,   default_ts_limits,   DataSampler::OnChannelLimit),
		Channel(21, gChannelsPins[21],  DataSampler::mSamples.data.raw[21],  Channel::Type::VREF, default_vref_limits, DataSampler::OnChannelLimit)
};

void SamplerTask(const void * argument);
osThreadDef(samplerTask, SamplerTask, osPriorityNormal, 0, 256);
...
void DataSampler::DoSamplerTask()
{
	...
	while( 1 ){
		Samples* sensorsData = (Samples*)osMailAlloc(*this, osWaitForever);
		if(sensorsData) {
			for (int i = 0; i < WL_CHANNEL_COUNT + 2; i++) {
				mChannels[i].Measure();
			}
			*sensorsData = mMav.Filter(&mSamples);

			gettimeofday(&sensorsData->timestamp,0);
			mTs = sensorsData->timestamp;
			osMailPut(*this, (void*)sensorsData);
		}
	}
}

void SamplerTask(const void * argument){
	static_cast<DataSampler*>(const_cast<void*>(argument))->DoSamplerTask();
}
